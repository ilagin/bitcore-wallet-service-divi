 ...
 * @param {string} opts.coin[='gbx'] - The coin for this wallet (gobyte).
 ...
 * @param {string} opts.coin[='gbx'] - The expected coin for this wallet (gobyte).
 ...
 * @param {Object} opts
 * @param {String} [opts.coin='gbx'] (optional)
    ...
    self._getUtxosForCurrentWallet({}, function(err, utxos) {
      ...
      var info = {
       ...
       if(opts.isInstantSend===true){
        ...
        if(!validUTXO)
            return cb(new Error('InstantSend requires all Gobyte you are sending (transaction inputs) to be held for about 15-20 minutes (minimum 6 network confirmations). Please wait before using InstantSend.'));
       }
 ...
 * @param {Object} opts
 * @param {string} [opts.coin = 'gbx'] - The coin to estimate fee levels from.
...
WalletService.prototype._selectTxInputs = function(txp, utxosToExclude, cb, isInstantSend) {
  ...
  function select(utxos, cb) {
    ...
    if (totalValueInUtxos < txpAmount) {
      self.logi('Total value in all utxos (' + Utils.formatAmountInGbx(totalValueInUtxos) + ') is insufficient to cover for txp amount (' + Utils.formatAmountInGbx(txpAmount) + ')');
      return cb(Errors.INSUFFICIENT_FUNDS);
    }
    if (netValueInUtxos < txpAmount) {
      self.logi('Value after fees in all utxos (' + Utils.formatAmountInGbx(netValueInUtxos) + ') is insufficient to cover for txp amount (' + Utils.formatAmountInGbx(txpAmount) + ')');
    ...
    self.logi('Big input threshold ' + Utils.formatAmountInGbx(bigInputThreshold));
    ...
    _.each(smallInputs, function(input, i) {
      ...
      //log.debug('The input contributes ' + Utils.formatAmountInGbx(netInputAmount));
      ...
      //log.debug('Tx size: ' + Utils.formatSize(txpSize) + ', Tx fee: ' + Utils.formatAmountInGbx(fee));
      ...
      if (!_.isEmpty(bigInputs)) {
        ...
        if (feeVsAmountRatio > Defaults.UTXO_SELECTION_MAX_FEE_VS_TX_AMOUNT_FACTOR) {
          ...
          //log.debug('Fee/Single-input fee: ' + Utils.formatRatio(feeVsSingleInputFeeRatio) + ' (max: ' + Utils.formatRatio(Defaults.UTXO_SELECTION_MAX_FEE_VS_SINGLE_UTXO_FEE_FACTOR) + ')' + ' loses wrt single-input tx: ' + Utils.formatAmountInGbx((selected.length - 1) * feePerInput));
      ...
      //log.debug('Cumuled total so far: ' + Utils.formatAmountInGbx(total) + ', Net total so far: ' + Utils.formatAmountInGbx(netTotal));

      if (netTotal >= txpAmount) {
        var changeAmount = Math.round(total - txpAmount - fee);
        //log.debug('Tx change: ', Utils.formatAmountInGbx(changeAmount));

        var dustThreshold = Math.max(Defaults.MIN_OUTPUT_AMOUNT, Bitcore.Transaction.DUST_AMOUNT);
        if (changeAmount > 0 && changeAmount <= dustThreshold) {
          //log.debug('Change below dust threshold (' + Utils.formatAmountInGbx(dustThreshold) + '). Incrementing fee to remove change.');
    ...
    if (netTotal < txpAmount) {
      //log.debug('Could not reach Txp total (' + Utils.formatAmountInGbx(txpAmount) + '), still missing: ' + Utils.formatAmountInGbx(txpAmount - netTotal));
  ...
  //log.debug('Selecting inputs for a ' + Utils.formatAmountInGbx(txp.getTotalAmount()) + ' txp');
  ...
  self._getUtxosForCurrentWallet({}, function(err, utxos) {
    ...
    async.whilst(function() {
      ...
      select(candidateUtxos, function(err, selectedInputs, selectedFee) {
        ...
        //log.debug('Fee for this selection: ' + Utils.formatAmountInGbx(fee));
      ...
      if (!err) {
        ...
        self.logi('Successfully built transaction. Total fees: ' + Utils.formatAmountInGbx(txp.fee) + ', total change: ' + Utils.formatAmountInGbx(change));
      } else {
 ...
 * @param {Object} opts
 * @param {string} [opts.coin = 'gbx'] - The coin for this transaction.
 ...
 * @param {Object} opts
 * @param {string} [opts.network = 'livenet'] - The Gobyte network for this transaction.
 * @param {string} opts.rawTx - Raw tx data.
 */
