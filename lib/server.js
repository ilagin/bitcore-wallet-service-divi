...
WalletService.prototype.savePreferences = function(opts, cb) {
  ...
  var preferences = [{
    ...
    name: 'unit',
    isValid: function(value) {
      return _.isString(value) && _.contains(['gbx'], value.toLowerCase());
    },
  }];
...
WalletService.prototype._selectTxInputs = function(txp, utxosToExclude, cb, isInstantSend) {
  ...
  function select(utxos, cb) {
    ...
    if (totalValueInUtxos < txpAmount) {
      self.logi('Total value in all utxos (' + Utils.formatAmountInDivi(totalValueInUtxos) + ') is insufficient to cover for txp amount (' + Utils.formatAmountInGbx(txpAmount) + ')');
      return cb(Errors.INSUFFICIENT_FUNDS);
    }
    if (netValueInUtxos < txpAmount) {
      self.logi('Value after fees in all utxos (' + Utils.formatAmountInDivi(netValueInUtxos) + ') is insufficient to cover for txp amount (' + Utils.formatAmountInGbx(txpAmount) + ')');
